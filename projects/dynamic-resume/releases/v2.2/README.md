...
```bash
python build_resume_v2.py resume.md resume.html
```
...
## Update: Header placement

This variant renders the resume header (name + contact info) at the top of the main content (above the Links section) instead of in the sidebar navigation, so it prints correctly and reads naturally on mobile/desktop.
